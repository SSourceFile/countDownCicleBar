...
/**
 * Created by hmh on 2018/8/4.
 * 倒计时动画
 */

public class ApplyCountDownView extends View {
    ...
    private int mPaintArcColor = Color.parseColor("#D5E4E1");//初始值
    private int mPaintBackGroundColor = Color.parseColor("#D5E4E1");//初始值
    private int mLoadingTime;//时间，单位秒
    private String mLoadingTimeUnit = "秒";//时间单位
    ...
    public ApplyCountDownView(Context context) {
        this(context, null);
    }

    public ApplyCountDownView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public ApplyCountDownView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        innerColor = array.getColor(R.styleable.CountDownView_cd_inner_color, Color.WHITE);
    ...
    private void init() {
        ...
        mPaintBackGround.setColor(innerColor);
...
//        RectF rectF1  = new RectF(getWidth()/2-mCircleRadius+mPaintArcWidth/2,getHeight()/2-mCircleRadius+mPaintArcWidth/2
//                ,getWidth()/2+mCircleRadius-mPaintArcWidth/2,getHeight()/2+mCircleRadius-mPaintArcWidth/2);
//        canvas.drawArc(rectF1, 0, 360, false, mPaintBackGround);
        //绘制动态圆弧
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        mRectF = new RectF(0 + mPaintArcWidth / 2, 0 + mPaintArcWidth / 2
                , mWidth - mPaintArcWidth / 2, mHeight - mPaintArcWidth / 2);
    }

    private void drawNormalRing(Canvas canvas) {
        Paint ringNormalPaint = new Paint(mPaintBackGround);
        ringNormalPaint.setStyle(Paint.Style.STROKE);
        ringNormalPaint.setStrokeWidth(mPaintArcWidth);
        ringNormalPaint.setColor(ContextCompat.getColor(getContext(), R.color.c_d5e4e1));//圆环默认颜色为灰色
